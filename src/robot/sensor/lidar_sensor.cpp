...
namespace fs {
    ...
    LIDARSensor::LIDARSensor(std::shared_ptr<muli::World> world, ScanPattern pattern, double frequency, double min_r,
                             double max_r, double h_fov, double h_res)
        : physics_world(world), scan_pattern(pattern), update_frequency(frequency), next_update_time(0.0),
          min_range(min_r), max_range(max_r), range_accuracy(0.01),
          horizontal_fov(h_fov * M_PI / 180.0) // Convert to radians
          ,
          vertical_fov(0.0), horizontal_resolution(h_res * M_PI / 180.0) // Convert to radians
          ,
          vertical_resolution(1.0 * M_PI / 180.0), current_scan_angle(0.0), current_beam_index(0),
          scan_in_progress(false), scan_start_time(0.0), range_noise_std(0.01),
          beam_divergence(0.1 * M_PI / 180.0) // 0.1 degrees
          ,
          dropout_probability(0.01), false_positive_rate(0.001), simulate_weather(false), rain_intensity(0.0),
          fog_density(0.0), dust_level(0.0) {
    ...
    bool LIDARSensor::is_data_valid() const { return data_valid; }
    ...
    void LIDARSensor::set_sensor_offset(const concord::Pose &offset) { sensor_offset = offset; }
    ...
    void LIDARSensor::enable_weather_simulation(bool enable) { simulate_weather = enable; }

    void LIDARSensor::set_collision_filter(const muli::CollisionFilter &new_filter) { filter = new_filter; }
    ...
    bool LIDARSensor::is_scanning() const { return scan_in_progress; }
        ...
        for (size_t i = 0; i < current_data.ranges.size(); ++i) {
            if (current_data.valid[i]) {
                if (transform_to_world) {
                    muli::Vec2 world_point = calculate_world_position(current_data.angles[i], current_data.ranges[i]);
    ...
    muli::Vec2 LIDARSensor::get_sensor_world_position() const {
        ...
        double world_x = robot_pose.point.x + (sensor_offset.point.x * cos_yaw - sensor_offset.point.y * sin_yaw);
        double world_y = robot_pose.point.y + (sensor_offset.point.x * sin_yaw + sensor_offset.point.y * cos_yaw);

        return muli::Vec2(world_x, world_y);
    }

    double LIDARSensor::get_sensor_world_orientation() const { return robot_pose.angle.yaw + sensor_offset.angle.yaw; }
