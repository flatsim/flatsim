...
ifeq ($(BUILD_SYSTEM),zig)
    ...
    CMD_BUILD       := cd $(BUILD_DIR) && make -j$(shell nproc) --no-print-directory 2>&1 | tee "$(TOP_DIR)/.complog"
    CMD_CONFIG      := mkdir -p $(BUILD_DIR) && cd $(BUILD_DIR) && if [ -f Makefile ]; then make clean --no-print-directory; fi && cmake -Wno-dev $(CMAKE_COMPILER_FLAG) $(CMAKE_BIG_TRANSFER_FLAG) -D$(PROJECT_CAP)_BUILD_EXAMPLES=ON -D$(PROJECT_CAP)_ENABLE_TESTS=ON .. 2>&1 | tee "$(TOP_DIR)/.complog"
