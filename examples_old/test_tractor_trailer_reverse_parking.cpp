...
int main(int argc, char *argv[]) {
    ...
    tractor.tracker->set_controller_type(drivekit::TrackerType::MPC_TRAILER);

    auto mpc_controller = dynamic_cast<drivekit::pred::MPCTrailerFollower *>(tractor.tracker->get_controller());
    if (mpc_controller) {
    ...
    drivekit::PathGoal path(reverse_parking_path, 1.0f, 0.6f, false); // Tight positional tolerance, modest speed
    ...
    while (true) {
        ...
        if (reverse_phase && tractor.tracker->is_path_completed()) {
            ...
            drivekit::PathGoal forward_goal(forward_path, 0.8f, 0.6f, false);
