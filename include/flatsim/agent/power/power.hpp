    ...
    class Power {
      ...
      public:
        Power(const std::string &name, Type type, float capacity, float consumption_rate, float charge_rate = 0.0f)
            : name(name), type(type), capacity(capacity), current_amount(capacity), consumption_rate(consumption_rate),
        ...
        void refuel(float amount) { current_amount = std::min(current_amount + amount, capacity); }

        void refuel_full() { current_amount = capacity; }

        float get_percentage() const { return capacity > 0.0f ? (current_amount / capacity) * 100.0f : 0.0f; }

        bool is_empty() const { return current_amount <= 0.0f; }

        bool is_low() const {
            return get_percentage() < 15.0f; // Warning threshold at 15%
        }

        bool is_full() const { return current_amount >= capacity; }
