...
namespace fs {
    ...
    class Tank {
      ...
      public:
        ...
        void empty(float amount) { current_amount = std::max(current_amount - amount, 0.0f); }

        void empty_all() { current_amount = 0.0f; }
        ...
        void visualize(std::shared_ptr<rerun::RecordingStream> rec) {
                ...
                rec->log_static(
                    ...
                    rerun::Boxes3D::from_centers_and_sizes({{filled_x, filled_y, 0.12f}}, {{t_w, filled_height, 0.0f}})
                        ...
                        .with_rotation_axis_angles(
                            {rerun::RotationAxisAngle({0.0f, 0.0f, 1.0f}, rerun::Angle::radians(t_th))})
            ...
            rec->log_static(parent_name + "/tank/" + name + "/outline",
                            rerun::Boxes3D::from_centers_and_sizes({{t_x, t_y, 0.11f}}, {{t_w, t_h, 0.0f}})
                                ...
                                .with_rotation_axis_angles(
                                    {rerun::RotationAxisAngle({0.0f, 0.0f, 1.0f}, rerun::Angle::radians(t_th))})
                                .with_colors({rerun::Color(80, 80, 80)}));
        }
    };
