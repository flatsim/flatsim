...
namespace mvs {
    ...
    inline mvs::RobotInfo tractor(concord::Pose pose, std::string name, pigment::RGB color = pigment::RGB(255, 200, 0),
        ...
        // wheels: front
        std::vector<concord::Bound> wheels;
        concord::Size front_w_size{width * 0.22f, height * 0.26f, 0.0f};
        wheels.push_back(concord::Bound(concord::Pose(width / 2, (height / 2) * 0.6f, 0.0f), front_w_size));
        wheels.push_back(concord::Bound(concord::Pose(-width / 2, (height / 2) * 0.6f, 0.0f), front_w_size));

        // wheels: back
        concord::Size back_w_size{width * 0.36f, height * 0.5f, 0.0f};
        ...
        // control limits
        std::vector<float> steerings_max = {deg2rad(45), deg2rad(45), 0.0f, 0.0f};
        std::vector<float> throttles_max = {0.0f, 0.0f, 5.0f, 5.0f};
        std::vector<float> steerings_diff = {deg2rad(2), deg2rad(2), 0.0f, 0.0f};
        robot_info.controlz = {steerings_max, throttles_max, steerings_diff};

        // karosserie
        concord::Size k_size = concord::Size(width * 0.40f, height * 0.10f, 0.0f);
        robot_info.karosseriez["front"] =
            concord::Bound(concord::Pose(0.0f, (height / 2) + k_size.y / 2, 0.0f), k_size);
