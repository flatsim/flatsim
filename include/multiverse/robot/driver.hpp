    ...
    class Vehicle {
      public:
        Vehicle(muli::World *world, const concord::Pose &pose) : world(world) {
            ...
            wheels[0].init(world, s, tf_fr, filter, linearDamping, angularDamping, force, friction, maxImpulse, brake,
                           drag);
            Transform tf_fl(Vec2(p.x - w / 2, p.y + h / 2));
            wheels[1].init(world, s, tf_fl, filter, linearDamping, angularDamping, force, friction, maxImpulse, brake,
                           drag);

            // Rear wheels
            Transform tf_rr(Vec2(p.x + w / 2, p.y - h / 2));
            wheels[2].init(world, s, tf_rr, filter, linearDamping, angularDamping, force, friction, maxImpulse, brake,
                           drag);
            Transform tf_rl(Vec2(p.x - w / 2, p.y - h / 2));
            wheels[3].init(world, s, tf_rl, filter, linearDamping, angularDamping, force, friction, maxImpulse, brake,
                           drag);
            ...
            for (int32 i = 0; i < 4; ++i) {
                joints[i] = world->CreateMotorJoint(body, wheels[i].wheel, wheels[i].wheel->GetPosition(), mf, torque,
                                                    fr, dr, jm);
            }
        }
