...
int main(int argc, char *argv[]) {
    ...
    tractor.tracker->set_controller_type(navcon::TrackerType::SOC);

    // Access the SOC controller directly to configure it
    auto soc_controller = dynamic_cast<navcon::pred::SOCFollower *>(tractor.tracker->get_controller());
    if (soc_controller) {
    ...
    navcon::PathGoal path(s_curve_path, 2.0f, 2.0f, false); // Reasonable tolerances
