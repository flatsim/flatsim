...
int main(int argc, char **argv) {
    ...
    std::filesystem::path machine_file = "examples/machines/oxbo_harvester.json";
    ...
    auto &harvester = sim.spawn_agent(machine_file, concord::Pose(0.0, 0.0, 0.0f), "carrot_harvester_0");
    ...
    while (!harvester.tracker()->is_path_completed()) {
        ...
        if (step_count % 120 == 0) {
            ...
            std::cout << (step_count / 60) << "s: Target(" << target.x << "," << target.y << "), Harvester("
                      << pos.point.x << "," << pos.point.y << "), Yaw=" << pos.angle.yaw << ", Dist=" << dist << "m\n";
        }
