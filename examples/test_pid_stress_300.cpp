...
int main(int argc, char *argv[]) {
    std::cout << "=== PID Controller Stress Test: 300 Robots ===" << std::endl;

    auto rec = std::make_shared<rerun::RecordingStream>("pid_stress_300", "space");
    ...
    concord::Size world_size{1500.0f, 1500.0f, 300.0f};
    simulator.init(world_datum, world_size);

    const int NUM_ROBOTS = 300;
    const int GRID_SIZE = 20; // 20x15 grid for 300 robots
    ...
    for (int i = 0; i < NUM_ROBOTS; ++i) {
        int row = i / GRID_SIZE;
        int col = i % GRID_SIZE;
        float x = -500.0f + col * SPACING;
        float y = -400.0f + row * SPACING;
        spawn_positions.push_back({x, y});
    ...
    std::cout << "All " << NUM_ROBOTS << " tractors loaded in " << load_duration << "ms\n" << std::endl;
    ...
    for (int i = 0; i < NUM_ROBOTS; ++i) {
        ...
        // Set S-shape path from spawn position
        auto path = generate_s_shape(spawn_positions[i].x, spawn_positions[i].y, 1.0f);
        waypoint::PathGoal path_goal(path, 2.0f, 2.5f, false);
        tractor.tracker->set_path(path_goal);

        if ((i + 1) % 100 == 0) {
            std::cout << "Configured " << (i + 1) << "/" << NUM_ROBOTS << " paths" << std::endl;
        }
    }
    ...
    while (!all_completed) {
        ...
        if (elapsed > 600) { // 10 minute timeout for 300 robots
        ...
        if (step_count % 10 == 0) {
            tick_times.push_back(tick_duration / 1000.0f);
        }
        ...
        if (step_count % 120 == 0) {
            float avg_tick_time = tick_duration / 1000.0f;
            float fps = 1000.0f / avg_tick_time;
            std::cout << "\n--- Progress at " << step_count / 60 << "s ---" << std::endl;
            std::cout << "  Tick: " << avg_tick_time << "ms (" << fps << " FPS)" << std::endl;
            std::cout << "  Completed: " << completed_count << "/" << NUM_ROBOTS << " ("
                      << (100.0f * completed_count / NUM_ROBOTS) << "%)" << std::endl;

            // Show sample of first 3 robots only
            for (int i = 0; i < std::min(3, NUM_ROBOTS); ++i) {
                auto &tractor = simulator.get_robot(i);
    ...
    std::cout << "\n=== PID Controller Stress Test Results (300 Robots) ===" << std::endl;
    ...
    float min_tick = *std::min_element(tick_times.begin(), tick_times.end());
    float max_tick = *std::max_element(tick_times.begin(), tick_times.end());
    float avg_tick = std::accumulate(tick_times.begin(), tick_times.end(), 0.0f) / tick_times.size();
    ...
    std::cout << "  Min Tick Time: " << min_tick << "ms" << std::endl;
    std::cout << "  Max Tick Time: " << max_tick << "ms" << std::endl;
    std::cout << "  Avg Tick Time: " << avg_tick << "ms (" << (1000.0f / avg_tick) << " FPS)" << std::endl;
