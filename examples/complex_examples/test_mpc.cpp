// MPC Path Following Test
//
// Run:
//   ./build/test_mpc

#include "flatsim/agent.hpp"
#include "flatsim/simulator.hpp"
#include "flatsim/utils.hpp"
#include <chrono>
#include <cmath>
#include <drivekit.hpp>
#include <iostream>
#include <thread>
#include <vector>

int main() {
    std::cout << "=== MPC Path Following Test ===" << std::endl;

    datapod::Geo datum{51.98954034749562, 5.6584737410504715, 53.801823};
    simulator::Simulator sim(200.0f, 200.0f, datum);

    datapod::Pose spawn_pose = utils::make_pose_2d(0.0, 0.0, -1.5708f);
    auto &tractor = sim.spawn_agent("machines/urdf/tractor.urdf", spawn_pose, "mpc_0");
    std::cout << "Tractor loaded: " << tractor.name() << " (" << tractor.uuid() << ")\n";

    std::cout << "\n--- Testing MPC Controller with S-Curve Path ---" << std::endl;

    tractor.tracker()->set_controller_type(drivekit::TrackerType::MPC);
    tractor.set_tracker_enabled(true);

    auto *mpc = dynamic_cast<drivekit::pred::MPCFollower *>(tractor.tracker()->get_controller());
    if (!mpc) {
        std::cerr << "[Error] Failed to cast to MPC controller!" << std::endl;
        return 1;
    }

    auto mpc_config = mpc->get_mpc_config();
    mpc_config.horizon_steps = 20;
    mpc_config.dt = 0.05;
    mpc_config.ref_velocity = 0.8;
    mpc_config.weight_cte = 200.0;
    mpc_config.weight_epsi = 150.0;
    mpc_config.weight_vel = 1.0;
    mpc_config.weight_steering = 50.0;
    mpc_config.weight_acceleration = 20.0;
    mpc_config.weight_steering_rate = 800.0;
    mpc_config.weight_acceleration_rate = 100.0;
    mpc->set_mpc_config(mpc_config);

    std::cout << "MPC Config: horizon=" << mpc_config.horizon_steps << " steps, ref_v=" << mpc_config.ref_velocity
              << " m/s\n";

    std::vector<datapod::Point> s_curve_waypoints = {
        {0.0f, 0.0f},   {5.0f, 0.0f},   {10.0f, 1.0f},  {15.0f, 3.0f},  {20.0f, 6.0f},  {25.0f, 10.0f}, {30.0f, 14.0f},
        {35.0f, 17.0f}, {40.0f, 19.0f}, {45.0f, 20.0f}, {50.0f, 19.0f}, {55.0f, 17.0f}, {60.0f, 14.0f}, {65.0f, 10.0f},
        {70.0f, 6.0f},  {75.0f, 3.0f},  {80.0f, 1.0f},  {85.0f, 0.0f},  {90.0f, 0.0f}};

    tractor.tracker()->set_path(drivekit::PathGoal(s_curve_waypoints, 2.0f, 2.0f, false));
    tractor.tracker()->smoothen(25.0f);

    std::cout << "Path set with " << s_curve_waypoints.size() << " waypoints\n";
    std::cout << "Starting MPC path following...\n" << std::endl;

    int step_count = 0;
    auto start_time = std::chrono::steady_clock::now();
    const float dt = 0.016f;

    while (!tractor.tracker()->is_path_completed()) {
        auto current_time = std::chrono::steady_clock::now();
        auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(current_time - start_time).count();

        if (elapsed > 300) {
            std::cout << "Timeout reached!\n";
            break;
        }

        sim.tick(dt);
        sim.tock();

        if (step_count % 120 == 0) {
            auto status = mpc->get_status();
            auto pos = tractor.get_position();

            std::cout << (step_count / 60) << "s: Pos(" << pos.point.x << "," << pos.point.y << "), "
                      << "Yaw=" << utils::get_yaw(pos) << ", "
                      << "CTE=" << status.cross_track_error << "m, "
                      << "HeadingErr=" << (status.heading_error * 180.0 / M_PI) << "deg\n";
        }

        step_count++;
        std::this_thread::sleep_for(std::chrono::milliseconds(16));
    }

    if (tractor.tracker()->is_path_completed()) {
        std::cout << "\nMPC successfully completed S-curve path!\n";
        std::cout << "Total time: " << (step_count / 60.0f) << " seconds\n";
    } else {
        std::cout << "\nMPC did not complete path within timeout\n";
    }

    auto final_pos = tractor.get_position();
    std::cout << "Final position: (" << final_pos.point.x << ", " << final_pos.point.y << ")\n";

    return 0;
}
