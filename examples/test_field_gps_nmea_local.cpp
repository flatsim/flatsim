...
static std::vector<concord::Point> generate_dubins_path(const std::vector<std::shared_ptr<const farmtrax::Swath>> &swaths,
...
int main(int argc, char **argv) {
    ...
    poly.addPoint(concord::Point{-167.0, -167.0, 0.0});
    poly.addPoint(concord::Point{167.0, -167.0, 0.0});
    poly.addPoint(concord::Point{167.0, 167.0, 0.0});
    poly.addPoint(concord::Point{-167.0, 167.0, 0.0});
    poly.addPoint(concord::Point{-167.0, -167.0, 0.0});
    ...
    auto &tractor = sim.spawn_agent("examples/machines/tractor.json",
                                    concord::Pose(spawn_x, spawn_y, spawn_yaw - 1.5708f),
                                    uuid,
                                    color);
